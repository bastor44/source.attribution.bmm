...
if(args$local==1) args$job_dir <- file.path(args$outdir, paste0(args$stanModelFile, '-', args$job_tag, '-', args$job_id, '-2stage'))
args$savedata <- TRUE
...
clus_cols <- pal_brewer(palette='Accent')(5)[c(1,2,3,5)]
g <- ggplot(data.table(ams)) + 
  geom_point(aes(x=TIME_ELAPSED,y=GEN_DIST,col=factor(CLUSTER)), alpha=p.alpha) + 
  #scale_color_brewer(palette='Paired') + 
  scale_color_manual(values=clus_cols) +
  theme_bw(base_size = 14) + 
  labs(x='Time elapsed\n(years)',y='Genetic distance',col='Cluster',
       title='a) Cluster membership of Amsterdam data') +
  scale_x_continuous(expand=c(0,0), breaks = seq(0,20,5)) +
  scale_y_continuous(expand=c(0,0), breaks = seq(0,0.2,0.05)) +
...
ggsave(file=paste0(outfile.base, '-Ams_gmm_classifications', n_clusters, '.pdf'), g, w=7, h=5)
...
p2 <- ggplot(dps, aes(x = d_TSeqT)) +
  ...
  scale_color_manual(name="",values = c(pal[6], pal[3]),
                     labels=c('Unlinked pairs', 'True transmission pairs'),
                     guide=guide_legend()) +
...
p3 <- ggplot(data = dps,aes(x=d_TSeqT))+
  ...
  theme_bw(base_size=14)+
  scale_color_manual(name="",values = c(pal[3],'grey70',pal[6]),
                     labels=c('True transmission pairs','Unlinked pairs','False phylogenetic signal')) +
...
# all same colour
p4 <- ggplot(data = dps,aes(x=d_TSeqT))+
  geom_ribbon(data = dps, aes(ymin = q2.5, ymax = q10), fill = p.palette[1], alpha = p.alpha) +
  geom_ribbon(data = dps, aes(ymin = q90, ymax = q97.5), fill = p.palette[1], alpha = p.alpha) +
  geom_ribbon(data = dps, aes(ymin = q10, ymax = q20), fill = p.palette[2], alpha = p.alpha) +
  geom_ribbon(data = dps, aes(ymin = q80, ymax = q90), fill = p.palette[2], alpha = p.alpha) +
  ...
  geom_ribbon(data = dps, aes(ymin = q60, ymax = q70), fill = p.palette[4], alpha = p.alpha) +
  geom_ribbon(data = dps, aes(ymin = q40, ymax = q60), fill = p.palette[5], alpha = p.alpha) +
  geom_point(data=sim_scenario, aes(x=TIME_ELAPSED, y=GEN_DIST), color=pal[3], alpha=p.alpha) +
  theme_bw(base_size=14) + 
  labs(x='Time elapsed \n(years)',y='',
       title='b) Simulated data with 14% true pairs') +
  scale_x_continuous(expand = c(0,0)) +
  scale_y_continuous(expand = c(0,0)) +
  coord_cartesian(xlim=c(0,16),ylim=c(0,0.2))
p4
ggsave(file=file.path(out.dir, 'simulated_data_nocol.png'), p4, w=7, h=5)
ggsave(file=file.path(out.dir, 'simulated_data_nocol.pdf'), p4, w=7, h=5)

sim_plots <- g + p4
sim_plots
ggsave(file=file.path(out.dir, 'simulated_data_panel.png'), sim_plots, w=12, h=5)
ggsave(file=file.path(out.dir, 'simulated_data_panel.pdf'), sim_plots, w=12, h=5)

## generate stan data ----
stan_data <- list()

# GP-specific stan data 
...
### fit GMM after background subtraction ----
tmp <- sim_scenario 

# remove obs in the signal cone 
clock_cone <- dps |> 
  dplyr::select(c(d_TSeqT, q2.5, q97.5))
data_keep <- dplyr::select(tmp, c(TIME_ELAPSED, GEN_DIST, TRANSMISSION_PAIR, FALSE_POS))
data_keep[ , TE_ROUNDED:=round(data_keep$TIME_ELAPSED, 2)]
...
data <- data_keep |>
  #dplyr::select(TIME_ELAPSED, GEN_DIST, q2.5, q97.5, RM) |>
  filter(RM == 0)

# plot background density with blue signal cone 
line_col <- pal_brewer(palette='Accent')(8)[6]
bg_dens <- ggplot(dps, aes(x = d_TSeqT)) +
  geom_ribbon(data = dps, aes(ymin = q2.5, ymax = q10), fill = p.palette[1], alpha = p.alpha) +
  geom_ribbon(data = dps, aes(ymin = q90, ymax = q97.5), fill = p.palette[1], alpha = p.alpha) +
  geom_ribbon(data = dps, aes(ymin = q10, ymax = q20), fill = p.palette[2], alpha = p.alpha) +
  geom_ribbon(data = dps, aes(ymin = q80, ymax = q90), fill = p.palette[2], alpha = p.alpha) +
  ...
  geom_ribbon(data = dps, aes(ymin = q40, ymax = q60), fill = p.palette[5], alpha = p.alpha) +
  # background density 
  stat_density2d(data=data, aes(x=TIME_ELAPSED, y=GEN_DIST), colour=line_col) + 
  geom_point(data=data, aes(x=TIME_ELAPSED, y=GEN_DIST), alpha=p.alpha) +
  theme_bw(base_size = 14) + 
  scale_x_continuous(expand = c(0,0)) +
  scale_y_continuous(expand = c(0,0)) +
  coord_cartesian(xlim=c(0,16),ylim=c(0,0.2)) + 
  #lims(y=c(0, 0.2), x=c(0, 16)) +
  labs(x='Time Elapsed \n(years)', y='Genetic Distance \n(substitutions per nucleotide site)', 
       title='a) Empirical Density')
bg_dens
ggsave(file=paste0(outfile.base, '-emp_bg_density', '.png'), bg_dens, w=7, h=5)
ggsave(file=paste0(outfile.base, '-emp_bg_density', '.pdf'), bg_dens, w=7, h=5)


data <- matrix(c(data$TIME_ELAPSED, data$GEN_DIST), ncol=2)

sim_cem <- fit_cEM(data=data, kmin=1, kmax=20)
n_clust2 <- sim_cem$k

gmm_stage2 <- list(nclass=sim_cem$k, pi=sim_cem$pi, Mu=sim_cem$Mu, LTSigma=sim_cem$C, p=2)

bg_clust <- emcluster(data, pi=gmm_stage2$pi, Mu=gmm_stage2$Mu, LTSigma = variance2LTSigma(gmm_stage2$LTSigma), assign.class = T)

bg <- as.data.table(data)
bg[, CLUSTER:=bg_clust$class]

# plot GMM clusters
g <- ggplot(bg) + 
  geom_point(aes(x=V1, y=V2, color=factor(CLUSTER))) + 
  scale_color_discrete() + 
  theme_bw() + 
  coord_trans(ylim = c(0,0.2), xlim = c(0,16)) +
  labs(x='Time elapsed',y='Genetic distance',col='cluster membership')
g
ggsave(file = paste0(outfile.base,'-second_stage_GMM_classifications', n_clust2, '.png'),g, w = 7, h = 5)



# plot GMM density (fitted background GMM) 
te <- seq(0, 16, 0.1)
gd <- seq(0, 0.2, 0.01)
grid <- expand.grid(x1=te, x2=gd)

#sigma <- LTSigma2variance(gmm_stage2$LTSigma)
sigma <- gmm_stage2$LTSigma

grid$density <- gmm_density(as.matrix(grid), pi=gmm_stage2$pi, mu=gmm_stage2$Mu, sigma=sigma, K=n_clust2)

gmm_dens <- ggplot(dps, aes(x = d_TSeqT)) +
  geom_ribbon(data = dps, aes(ymin = q2.5, ymax = q10), fill = p.palette[1], alpha = p.alpha) +
  geom_ribbon(data = dps, aes(ymin = q90, ymax = q97.5), fill = p.palette[1], alpha = p.alpha) +
  geom_ribbon(data = dps, aes(ymin = q10, ymax = q20), fill = p.palette[2], alpha = p.alpha) +
  geom_ribbon(data = dps, aes(ymin = q80, ymax = q90), fill = p.palette[2], alpha = p.alpha) +
  ...
  geom_ribbon(data = dps, aes(ymin = q40, ymax = q60), fill = p.palette[5], alpha = p.alpha) +
  # gmm density 
  geom_contour(data=grid, aes(x=x1, y=x2, z=density), colour=line_col) +
  geom_point(data=data.table(data), aes(x=V1, y=V2), alpha=p.alpha) +
  theme_bw(base_size=14) + 
  lims(y=c(0, 0.2), x=c(0, 16)) +
  labs(x='Time Elapsed \n(years)', y='',
       title=paste('b) Density of fit GMM (k=', n_clust2, ')'))
gmm_dens
ggsave(file=paste0(outfile.base, '-gmm_density.png'), gmm_dens, w=7, h=5)
ggsave(file=paste0(outfile.base, '-gmm_density.pdf'), gmm_dens, w=7, h=5)

dens_plots <- bg_dens + gmm_dens
ggsave(file=paste0(outfile.base, '-density-panel.png'), dens_plots, w=12, h=5)
ggsave(file=paste0(outfile.base, '-density-panel.pdf'), dens_plots, w=12, h=5)

# set gmm parameters in stan data
#sigma <- LTSigma2variance(gmm_stage2$LTSigma)
sigma <- gmm_stage2$LTSigma

stan_data$K <- gmm_stage2$nclass
stan_data$gmm_pi <- gmm_stage2$pi
stan_data$mu <- t(gmm_stage2$Mu)
stan_data$Sigma <- array(sigma[, , ], dim = c(ncol(data), ncol(data), gmm_stage2$nclass))
stan_data$Sigma <- aperm(stan_data$Sigma, c(3, 1, 2))
stan_data$D <- 2
#stan_data$L <- rep(args$B*max(sim_scenario$FROM_AGE_STD,do$FROM_AGE_STD),2)
...
# cat(paste("posterior y_mix_0: ", round(inv.logit(su$median[1]), 3), "90% CI:", round(inv.logit(su$q5[1]), 3), ',', round(inv.logit(su$q95[1]), 3)))
...
# po <- po[,
#          list( q = quantile(value, probs = c(0.5, 0.025, 0.25, 0.75, 0.975), na.rm=TRUE ),
...
# ggsave(filename = paste0(outfile.base, '-posterior_prob_sizes.png'), p1, w=12, h=7)
# 
# ### violin plot(s) ----
# pal <- pal_npg("nrc")(5)
# 
...
# g3
# ggsave(filename=paste0(outfile.base, '-violin-posterior-prob.png'), h=5, w=6)
