...
real gmm_log_pdf(vector X, int K, vector gmm_pi, matrix mu, array[] matrix Sigma){ 
...
model{
 ...
 for(i in 1:M[1]){
  ...
  for (i in 1:N){
    target += log_mix(y_mix[i], gamma_lpdf(y[i] | tpair_alpha[i], tpair_beta[i]), gmm_log_pdf(X[i,]', K, gmm_pi, mu, Sigma)); //Background pairs given gmm
  }
}
    ...
    if(npairs>1){
      ...
      { // add llk of other pairs for same recipient being non-pairs
          if(i!=j && pt_map[pt_idx[i],j]==1){
            ...
            num += gmm_log_pdf(X[j,]', K, gmm_pi, mu, Sigma) + log1m(y_mix[i]);
            k += 1;
          }
       }
    }
    ...
    if(npairs>1){
      ...
      { // all combinations of pairs with other non-pairs
          if(pt_map[pt_idx[i],j]==1){
              ...
              den[npairs+1] = gmm_log_pdf(X[j,]', K, gmm_pi, mu, Sigma) + log1m(y_mix[i]);
            real bkg_sum=0; 
            for(l in 1:N)
            {
              if(j!=l && pt_map[pt_idx[i],l]==1){
                ...
                //den[npairs+1] += unif_lpdf + log1m(y_mix[i]);
                den[k] += gmm_log_pdf(X[l,]', K, gmm_pi, mu, Sigma) + log1m(y_mix[i]); 
                den[npairs+1] += gmm_log_pdf(X[l,]', K, gmm_pi, mu, Sigma) + log1m(y_mix[i]);
              }
           }
            k += 1;
          }
      ...
      den_one[2] = gmm_log_pdf(X[i,]', K, gmm_pi, mu, Sigma) + log1m(y_mix[i]); 
    }
    ...
    tpair_prob[i] = exp(
          ...
          (
            log_mix( y_mix[i],
              gamma_lpdf(y[i] | tpair_alpha[i], tpair_beta[i]),
              //unif_lpdf
              gmm_log_pdf(X[i,]', K, gmm_pi, mu, Sigma)
              )
          )
        );
